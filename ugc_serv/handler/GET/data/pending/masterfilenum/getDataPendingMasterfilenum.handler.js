        ...
        if (err) {
            ...
            bearer.verify(token, req.headers["username"], function (err) {
                if (err) {
                    ...
                    var sql = "SELECT DISTINCT master_file_num FROM plan_11_pending ORDER BY master_file_num asc";
                    db_conn.query(sql, function (err, result, fields) {
                        if (err) {
                            ...
                            for (var i = 0; i < result.length; i++)
                                responseObj.push(result[i].master_file_num != null ? "" + result[i].master_file_num : "(Blank)");
